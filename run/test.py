...
def main(args):
    ...
    for idx, doc in enumerate(tqdm(results, desc="Inferencing")):
        id = doc.get("id", "NIL")
        front= doc.get("input", {}).get("front", "")
        back= doc.get("input", {}).get("back", "")
            
        # 프롬프트 생성
        prompt = make_prompt(front, back, tokenizer)
        ...
        if response not in labels:
            if args.debug: print(f'front:{front}, back:{back} -> {response}', end=' -> ')
        ...
        # 디버깅용 출력
        if False and args.debug: print(f'front:{front}, back:{back} -> {response}')
